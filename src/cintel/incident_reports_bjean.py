"""
incident_reports_bjean.py - Project script (example).
...
- Data is taken from a system that records incident reporting metrics.
...
- The CSV file includes these columns:
  ...
  - level of seriousness, high, medium, low
...
Paths (relative to repo root)
    ...
    INPUT FILE: data/incident_reports.csv
    OUTPUT FILE: artifacts/signals_incident_reports.csv

Terminal command to run this file from the root project folder

    uv run python -m cintel.incident_reports_bjean
...
DATA_FILE: Final[Path] = DATA_DIR / "incident_reports.csv"
OUTPUT_FILE: Final[Path] = ARTIFACTS_DIR / "signals_incident_reports.csv"
...
def main() -> None:
    ...
    # ----------------------------------------------------
    # STEP 2.1: DEFINE THE COMPLIANCE STATUS SIGNAL RECIPE
    # ----------------------------------------------------
    # A signal recipe tells Polars how to build a new column.
    # If requests > 0, use errors / requests.
    # Otherwise, use 0.0.
    # Name the new column "error_rate".
    compliant_subm_signal_recipe: pl.Expr = (
        pl.when(pl.col("days_diff") > 1)
        .then(pl.lit("Non Compliant"))
        .otherwise(pl.lit("Compliant"))
        .alias("compliant_submission")
    )

    # ----------------------------------------------------
    # STEP 2.2: APPLY THE SIGNAL RECIPES TO THE DATAFRAME
    # ----------------------------------------------------
    # Now we use with_columns() to apply all the recipes
    # and create a new DataFrame with the added signal columns.
    df_with_signals: pl.DataFrame = df.with_columns([compliant_subm_signal_recipe])

    LOG.info("Created signal columns: compliant_submission")
